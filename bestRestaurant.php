<!doctype html>
<html lang="nl">
<head>
  <meta charset=utf-8>
  <title></title>
  <link rel="stylesheet" href="css/reset.css">
  <link rel="stylesheet" href="css/style.css">
  <link href="https://fonts.googleapis.com/css?family=Hind" rel="stylesheet">
</head>
<body>
  <div class="mainWrapper">
    <h1 class="subPageTitle">Stem van Leeuwarden</h1>
    <section id="bestRestaurantsSection">
      <h3 id="choiceTitle">Maak hieronder een keuze van de 3 beste restaurants in Leeuwarden</h3>

      <h6>Keuze 1</h6>
      <select name="choice1" id="choice1">
        <option value="" hidden="hidden">Maak een keuze</option>
        <option value="1">Restaurant1</option>
        <option value="2">Restaurant2</option>
        <option value="3">Restaurant3</option>
        <option value="4">Restaurant4</option>
        <option value="5">Restaurant5</option>
        <option value="6">Restaurant6</option>
        <option value="7">Restaurant7</option>
        <option value="8">Restaurant8</option>
      </select>

      <h6>Keuze 2</h6>
      <select name="choice2" id="choice1">
        <option value="" hidden="hidden">Maak een keuze</option>
      </select>

      <h6>Keuze 3</h6>
      <select name="choice3" id="choice1">
        <option value="" hidden="hidden">Maak een keuze</option>
      </select>
    </section>
    <div class="buttonCenter">
      <a href="#">
        <input type="button" id="startE" class="button gradientcolor" title="button"
        value="Volgende vraag">
      </a>
    </div>
  </div>
</body>
</html>
